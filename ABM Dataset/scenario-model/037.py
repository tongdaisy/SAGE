...
class Worker:
    ...
    def improve_skills(self):
        if self.money > 5 and random.uniform(0, 1) < 0.5:
            self.money -= 5
            self.skill *= 1.5
...
def simulate(num_workers, num_factories, num_steps):
    ...
    workers_salary_varience = np.var([w.salary for w in workers])
    return workers_salary_varience

    # Example usage


workers_salary_varience = simulate(100, 10, 100)*100
