# the electric vehicle driving and charging situation
# mesa
...
class EV(Agent):
    ...
    def __init__(self,
        ...
        self.trip_distance = \
            self.model.igraph.shortest_paths_dijkstra(source=self.model.nodes.index.get_loc(self.origin),
                                                      target=self.target,
    ...
    def charge_makes_sense(self):
        ...
        # need to update
        time_in_car = (
                              self.model.schedule.time - self.timestart) / 60  # time in hours since driving - need to check how this variable is affected after charging once
        charging_price = 0.0
        parking_price = 0.0
        ...
        # access_time
        # path path_to_cs
        menity_restroom = 1  # al charging stations have restrooms, this assumption needs validation
        restaurents = 1
        # amenity_more
        if restaurents > 0:
            amenity_more = 1
        ...
        if self.charge_station != None:
            ...
            charging_time = (max(80,
                                 self.charging_end_soc) - self.SOC) * self.capacity / 100 / station3.max_power  # energy used / power = time /hours
            ...
            if station3.dcfc_var_charging_price_unit == "min":
                ...
                charging_price = station3.dcfc_fixed_charging_price + ((max(80,
                                                                            self.charging_end_soc) - self.SOC) / 100 * self.capacity * station3.dcfc_var_charging_price / station3.max_power)
            charging_cost_in_dollar = parking_price + charging_price

            access_time = \
                self.model.igraph.shortest_paths_dijkstra(source=self.pos_igraph_id, target=self.charge_station,
                                                          weights='travel_time')[0][
                    0] / 60  # this converts the time to minutes

        # utility of charging
        u_charging = intercept + (c_soc * self.SOC / 100) + (c_dev * dev) + (c_time_in_car * time_in_car) + (
                c_charging_cost * charging_cost_in_dollar) + (c_charging_time * charging_time) + (
                             c_access_time * access_time) + (c_amenity_restroom * amenity_restroom) + (
                             c_amenity_more * amenity_more)
        ...
        if step_dis >= dis:
            ...
            if self.route_index + 1 < len(self.route) - 2:
                for route_i in range(self.route_index + 1, len(self.route) - 2):
                    ...
                    next_edg_dis = self.model.network.get_edge_data(self.route[route_i], self.route[route_i + 1])[0][
                        'length']
                    dis += next_edg_dis
                    if dis > step_dis:
                        break
            ...
            if self.route_index <= len(self.route) - 1:
                ...
                self.currentRoad = self.model.network.get_edge_data(self.route[route_i], self.route[route_i + 1])[0][
                    'osmid']
                # update speed
                self.veh_speed = self.model.network.get_edge_data(self.route[route_i], self.route[route_i + 1])[0][
                    'speed_kph']
            else:
                self.finish()
                self.veh_speed = 0
    ...
    def status_switch(self):
        ...
        if self.charge_decision_time == None or (
                self.model.schedule.time - self.charge_decision_time) > recondider_time:
            if self.status == 'driving':
                ...
                if self.SOC < min(self.charging_start_soc,
                                  MIN_SOC_CHARGING) or self.distance_to_des() > self.remaining_range or self.SOC < MAX_SOC_CHARGING:
                    ...
                    if self.charge_makes_sense() > 0:
                        ...
                        self.station_nodes = self.station_nodes[
                            self.station_nodes.values != self.model.nodes.iloc[self.charge_station].name]
...
class EVModel(Model):
    ...
    def __init__(
        ...
        for index, row in EV_agent.iterrows():
            t = EV(self.next_id(), row['soc'], row['oosmid'], row['olat'],
        ...
        for index, row in stations.iterrows():
            s = station(self.next_id(), row['dcfc_count'], row['dcfc_var_charging_price_unit'],
