# Simulate  a Bounded Confidence Model with finite Ages

import numpy as np
import numpy.random as npr
np.random.seed(1989) # for random events that are reproducible
...
class Population():
    ...
    def find_agents(self, mean, std):
        ...
        # input param mean mean opinion
        # input param std standard deviation of opinions in group
        # input return ID list of all Agents in bracket [mean-std, mean+std]
        step = 1.
        ids = []
    ...
    def get_groups(self, time, spread=smoothedge):
        ...
        # input param spread minimum distance between agents to classify as separate groups
        ids = []
        for ind in range(self.N):
    ...
    def add_agents(self, n, date=-maxage, opinion=-1):
        ...
        # input param n number
        # input param date current date, if not given: distribute ages uniformly within maxage
        # input param opinion fixed opinion; if not given: distribute opinions uniformly within [0,1]
        for i in range(n):
    ...
    def clear_old(self, date):
        ...
        # input param date current date, needed for age calculation
        mark_dead = []
        for i in range(self.N):
    ...
    # input param agent1 first agent
    # input param agent2 second agent
    # input param date current date
    # input param alpha weight of opinion difference, default is 1
    # input param beta  weight of age difference, default is 0 (neglect)
    # input param gamma weight of position difference, default is 0 (neglect)
    ...
    # quite weak as well
    # input param distance between to agents, float
    # input param epsilon relevant distance scale: below: preferring interaction
    # input param spread  allowance region for smoothing, in percent of epsilon


    def distofinfluence(self, spread=0.0):
        # determine interaction kernel size
        # input param spread = 0.0, Gaussian width of change of interaction strength
        if spread<1.e-30:
            return epsilon
    ...
    # input param date current simulation date
        self.clear_old(date)
        if (self.N <= 1):
...
class OpinionGroup():
    ...
    # input param meanOpinion float, mean opinion as reported by cluster.getlevel
    # input param stdOpinion float, opinion standard deviation as reported by cluster.getlevel
        self.ID = ID
        self.time = time
        self.Agents = agents
    ...
    # input param newgrouplist array of new groups
        N = len(newgrouplist)
...
class simulation():
    ...
    # input param time current simulation time
