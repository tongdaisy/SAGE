...
class Hospital:
    ...
    def admit_patient(self, people):
        new_treat = 0
        # admit according to the illness severity
        people.sort(key=lambda p: (p.infected,p.days_infected), reverse=True)
...
def simulation(population_size=100, infection_rate=0.2, recovery_day=5, simulation_days=20, hospital_capacity=10,
    ...
    for day in range(simulation_days):
        total_infect += government.simulate_spread(day)
    ...
    treatment_rate = treatment_num / total_infect
    return treatment_rate


treatment_rate = simulation()
